...
class _CalculatorScreenState extends State<CalculatorScreen>
  ...
  void _onDigit(String digit) {
    ...
    setState(() {
      ...
      if (_digitBuffer.length > _secretCode.length) {
        _digitBuffer = _digitBuffer.substring(
          _digitBuffer.length - _secretCode.length,
        );
      }
  ...
  // REPLACE _unlockVault() with:
  void _unlockVault() {
    _onClear();
    Navigator.pushNamedAndRemoveUntil(context, kRouteHome, (_) => false);
  }
  ...
  Widget _buildRow(List<_CalcBtn> buttons) {
    return Row(
      children: buttons.map((btn) {
        return Expanded(
          child: Padding(
            padding: EdgeInsets.only(right: btn == buttons.last ? 0 : 12),
            child: _buildButton(
              label: btn.label,
              onTap: btn.onTap,
              type: btn.type,
